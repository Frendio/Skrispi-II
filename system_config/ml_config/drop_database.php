<?php
    ...
    to('html', 'lang::en');
        to('head');
            title('MadeLine | Configuration | Drop Database');
                ...
                to('section', 'class::wy-nav-content-wrap');
                    to('div', 'class::wy-nav-content');
                        ...
                        to('div', 'id::content-holder');                    
                            to('form', 'class::drop_database ajax::1');
                                to('table', 'class::ml-table');                             
                                    to('tr', 'class::bb');
                                        to('td', 'class::td-table').show('Database Name').tc('td');
                                        to('td');
                                            to('select', 'class::ML-btn-full*ML-rounded*ML-border-gray*db-name name::db-name');
                                                to('option', "value::").show('Choose Database').tc('option');
                                                foreach($results as $row) {
                                                    if($fetch_type === 'object')
                                                    {
                                                        to('option', "value::$row->Database").show($row->Database).tc('option');
                                                    }
                                                    else
                                                    {
                                                        to('option', "value::$row[Database]").show($row['Database']).tc('option');
                                    ...
                                    to('tr');
                                        ...
                                        to('td');
                                            to('button', 'class::ganti_password*db_con').show('Drop Database').tc('button');
                                        tc('td');
                                    tc('tr');                                
                                tc('table');
                            tc('form');                        
