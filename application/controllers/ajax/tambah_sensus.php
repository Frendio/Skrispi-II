<?php
    ...
    class Tambah_sensus extends ML_Controller
    {
        ...
        public function index()
        {
            ...
            if($this->form->validate())
            {
                ...
                foreach($result as $row)
                {
                    ...
                    else if(strtolower($ns_sidi) === strtolower($row->no_surat_sidi))
                    {
                ...
                # insert new data kpp
                $this->model->data('nama_anggota', $nama_lengkap)
                            ->data('tempat_lahir', $tempat_lahir)
                            ->data('tanggal_lahir', $tanggal_lahir)
                            ->data('jenis_kelamin', $jenis_kelamin)
                            ->data('hub_keluarga', $hub_kel)
                            ...
                            ->data('cacat', $cacat)
                            ->data('go_darah', $gol_dara)
                            ->data('pend_terakhir', $ped_terakhir)
                            ->data('no_telfon', $nomor_telpon)
                            ->data('baptis', $baptis)
                            ->data('no_surat_baptis', $ns_baptis)
                            ->data('tanggal_baptis', $tgl_baptis)
                            ->data('nama_pendeta_baptis', $NP_baptpis)
                            ->data('jemaat_baptis', $jemaat_baptis)
                            ->data('sidi', $sidi)
                            ->data('no_surat_sidi', $ns_sidi)
                            ->data('tanggal_sidi', $tgl_sidi)
                            ->data('nama_pendeta_sidi', $NP_sidi)
                            ->data('jemaat_sidi', $jemaat_sidi)
                ...
                exit('1|Berhasil tambahkan anggota jemaat.');
            }
            else
            {
