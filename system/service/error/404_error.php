<?php 
    ...
    to('html');
        ...
        to('body','class::ml-no-bak');
            to('div', 'class::nf_error style::background:url('.BASE_URL.'system_config/ML_Assets/error/404.jpg)*center*no-repeat;*background-size:cover;').tc('div');
            to('div', 'class::ml-bholder');
                to('div', 'class::ml-iholder');
                    show('The page you are looking for is not found on this server!');
                    bl(2);
                tc('div');
                to('a', 'class::ns-link onclick::window.history.back();');
                    tag('button', 'class::pnf-button*ml-bbc*ml-button').show('Go Back').tc('button');
                tc('a');
                to('a', 'href::'.BASE_URL.'');
                    tag('button', 'class::pnf-button*ml-bbc*ml-button').show('Home').tc('button');
                tc('a');
            tc('div');
        tc('body');
    tc('html');
